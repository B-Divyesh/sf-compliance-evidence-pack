...
function licenseCard(compact = false): string {
  ...
  return `<section class="unlock-card ${compact ? 'compact' : ''}" aria-labelledby="unlock-title">
    ...
    <h2 id="unlock-title">Lifetime license</h2>
...
function landing(): string {
  ...
  <section class="two-up"><div><p class="eyebrow">Built for human review</p><h2>What Deadline Packet does not do.</h2><p>Deadline Packet doesn’t calculate tax, interpret rules, or submit anything. It keeps evidence gaps and accountant questions in the same packet.</p><ul class="ticks"><li>Stored in this browser</li><li>Attachments included in your ZIP</li><li>Evidence gaps follow the checklist</li><li>Open questions stay beside the files</li></ul></div>${licenseCard()}</section>
  ${createDialog()}`;
}

function notFound(): string {
  return `<section class="not-found"><p class="eyebrow neon">Error 404</p><div class="lost-folder" aria-hidden="true"><span></span><i></i><i></i><i></i></div><h1>Page not found.</h1><p>The address may be old or mistyped. Your local packet data has not changed.</p><a class="button" href="/" data-route>Return to Deadline Packet</a></section>`;
}
