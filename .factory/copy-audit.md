# Landing copy audit

Audited 2026-08-28. Word counts treat hyphenated terms and prices as one word.
No sentence exceeds 22 words. No banned marketing word is present; “unlock” is
used only for the literal paid license state below the first screen.

| Landing text | Words | Result |
| --- | ---: | --- |
| Evidence in order. | 3 | Pass |
| Questions in view. | 3 | Pass |
| Prepare evidence for your accountant. | 5 | Pass |
| For freelancers with cross-border income, turn one filing period’s invoices, receipts, gaps, and questions into an accountant-ready packet. | 18 | Pass |
| Try it with sample data | 5 | Pass |
| Start your packet | 3 | Pass |
| The sample opens as a complete working packet. | 8 | Pass |
| No account. | 2 | Pass |
| Data stays in this browser. | 5 | Pass |
| Works offline after your first visit. | 6 | Pass |
| One packet free. | 3 | Pass |
| Unlimited use costs US$19 once. | 5 | Pass |
| Import a JSON backup | 4 | Pass |
| From scattered evidence to one reviewable handoff. | 8 | Pass |
| Set the period | 3 | Pass |
| Choose your own handoff date. | 5 | Pass |
| Gather the proof | 3 | Pass |
| Mark gaps without guesswork. | 4 | Pass |
| Export one packet | 3 | Pass |
| ZIP + PDF index for review. | 6 | Pass |
| Built for human review | 4 | Pass |
| Not another filing portal. | 4 | Pass |
| Deadline Packet doesn’t calculate tax, interpret rules, or submit anything. | 10 | Pass |
| It helps you arrive at the accountant conversation with the evidence—and the unknowns—clearly labelled. | 14 | Pass |
| Local IndexedDB storage | 3 | Pass |
| Attachments included in your ZIP | 5 | Pass |
| Missing evidence listed automatically | 4 | Pass |
| Open questions kept beside the files | 6 | Pass |
| US$19 once. | 2 | Pass |
| Your first complete packet is free; unlimited packets and duplication require the lifetime license. | 14 | Pass |

## Terminology

| Concept | One term |
| --- | --- |
| A filing-period workspace | packet |
| A required document group not marked ready | evidence gap |
| The recipient | accountant |
| The downloadable archive | accountant ZIP |
| The isolated sample workspace | demo |
| The paid capability | lifetime license |
